$(function() {
	...
	$('#wrap-nav').load('html/nav.html', function() {
		...
		$('.shop').css('background-color','#46ab50')
...
//	右边轮播
	$li = $('.ta-maquee').find('li').clone();
	$('.ta-maquee').find('ul').append($li);
	//console.log($('.ta-maquee').find('ul').html())
	var timer = setInterval(function(){
		var ulTop = parseInt($('.ta-maquee').find('ul').css('top'))
		//console.log(ulTop)
		if(ulTop == -296){
			$('.ta-maquee').find('ul').css('top',0)
		}
		$('.ta-maquee').find('ul').finish().animate({
			'top':'-=74'
		},1000);
	},2000);
